...
float3 sdJoint2DFlat( in float2 p, in float l, in float a, float w)
{
    // if perfectly straight
    if( abs(a)<0.001 )
    {
...
float checker( in float2 p )
{
 ...
 //uv = 0.5*(uv-vec2(0.0,0.5));
    ...
    float3 duv1 = sdJoint2DSphere(uv-float2(-0.6,-0.3),le,an,wi);
    float3 duv2 = sdJoint2DFlat(  uv-float2( 0.6,-0.3),le,an,wi);
        
    float3 duv = (duv1.x<duv2.x) ? duv1 : duv2;
    ...
    if( d<0.0 )
    {
        col = checker(duv.yz*20.0) * float3(1,1,1);
        //col = sqrt(texture(iChannel0,duv.yz).xyz);
        col *= lerp(float3(0,0.6,1),float3(1,1,0),clamp(duv.z/le,0.0,1.0));
    }
