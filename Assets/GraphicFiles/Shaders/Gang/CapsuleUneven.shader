...
// uneven capsule
float sdUnevenCapsuleY( in float2 p, in float ra, in float rb, in float h )
{
	p.x = abs(p.x);
...
float sdUnevenCapsule( in float2 p, in float2 pa, in float2 pb, in float ra, in float rb )
{
    p  -= pa;
    pb -= pa;
    float h = dot(pb,pb);
...
// signed distance to a n-star polygon with external angle en
 ...
 //uv = 0.5*(uv-vec2(0.0,0.5));
    ...
    float2 v1 = cos( TIME + float2(0.0,2.00) + 0.0 );
	float2 v2 = cos( TIME + float2(0.0,1.50) + 1.5 );
    float r1 = 0.5+0.1*sin(TIME);
    float r2 = 0.3+0.1*sin(1.0 + 2.3 * TIME);
