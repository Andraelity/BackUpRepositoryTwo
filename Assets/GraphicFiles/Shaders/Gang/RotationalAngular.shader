...
float map_correct( in float2 p )
{
    ...
    float c1 = b*(i+0.0);
    float2x2 mat2 = {cos(c1),-sin(c1),sin(c1), cos(c1)};
	float2 p1 = mul(mat2,p); 

    float c2 = b*(i+1.0);
    float2x2 mat2_0 = {cos(c2),-sin(c2),sin(c2), cos(c2)};
    float2 p2 = mul(mat2_0,p);
