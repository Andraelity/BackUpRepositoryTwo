...
float terrain(float3 pos){   
	pos2.xz = pos.xz = fmod(pos.xz,400.0)-200.0;
	...
	return 	60.0 + sin(pos.x*0.12+time)*sin(pos.y*0.14+time)*sin(pos.z*0.11+time)*5.0 - length(pos);
 	...
 	time      	= TIME * 1.0;
    float2 uv     = coordinateScale - float2(1.0 + cos(time * 0.3) * 0.4, 0.9);
    float3 ray    = normalize(float3(uv.x,uv.y,0.7));
    float3 campos = float3(cos(time*0.32) * 100.0, sin(time * 0.5) * 20.0 - 300.0, cos(time*0.1)*500.0);
    float3 pos    = campos;
    ...
    float3 col = c2 * float3(0.9,0.3,0.0) + c3 * float3(0.1,0.4,0.6) + c1;
	float4 fragColor = float4(col-dot(uv,uv)*0.2,1.0);
