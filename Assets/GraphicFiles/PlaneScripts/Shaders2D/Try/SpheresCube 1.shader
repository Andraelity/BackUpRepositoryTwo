...
float map(float3 pos) {
	...
	ray.xy = mul(ray.xy, rot(sin(TIME * .03) * 5.));
	ray.yz = mul(ray.yz, rot(sin(TIME * .05) * .2));
	float t = 0.1;
	...
	col += float3(0.,0.2 * abs(sin(TIME)),0.5 + sin(TIME) * 0.2);


	float4 fragColor = float4(col ,1.0 - t * (0.02 + 0.02 * sin (TIME)));
