...
float noise2D(float2 uv)
{
	...
	return (lerp(lerp(r.x, r.y, f.x), lerp(r.z, r.w, f.x), f.y));	
            ...
            fixed4 frag (pixel i) : SV_Target
			{
				...
				float4 fragColor = float4(noise2D(coordinateScale * TIME)/abs(cos(TIME/2.0)),noise2D(coordinateScale * TIME)/abs(tan(TIME)), noise2D(coordinateScale * TIME)/abs(sin(TIME)), 1.0); 
